...
add-apt-repository ppa:ondrej/php # Add the Ondrej PPA for more recent PHP versions
apt update -y
...
echo "    try_files $uri $uri/ /index.php?$query_string;" >>/etc/nginx/conf.d/librenms.conf # Corrected line
echo "  }" >>/etc/nginx/conf.d/librenms.conf
